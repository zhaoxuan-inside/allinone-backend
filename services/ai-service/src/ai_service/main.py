...
from src.ai_service.router import router as ai_router

app = FastAPI(
    title="AI Service",
    description="AI Service for AllInOne Platform",
    version="0.1.0"
)
...
async def startup():
    ...
    UnifiedLogger.info("AI Service started successfully")
...
async def shutdown():
    ...
    if service_registry:
        await service_registry.deregister_service(service_name="ai-service")
        await service_registry.close()
    
    if config_center:
        await config_center.close()
    
    UnifiedLogger.info("AI Service shutdown completed")
