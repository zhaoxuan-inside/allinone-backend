...
from src.tools_service.router import router as tools_router

app = FastAPI(
    title="Tools Service",
    description="Tools Service for AllInOne Platform",
    version="0.1.0"
)
...
async def startup():
    ...
    UnifiedLogger.info("Tools Service started successfully")
...
async def shutdown():
    ...
    if service_registry:
        await service_registry.deregister_service(service_name="tools-service")
        await service_registry.close()
    
    if config_center:
        await config_center.close()
    
    UnifiedLogger.info("Tools Service shutdown completed")


@app.get("/")
async def root():
    return {"message": "Tools Service"}
