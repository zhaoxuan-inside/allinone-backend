...
app = FastAPI(
    title="News Service",
    description="News Service for AllInOne Platform",
    version="0.1.0"
)
...
async def startup():
    ...
    UnifiedLogger.info("News Service started successfully")
...
async def shutdown():
    ...
    if service_registry:
        await service_registry.deregister_service(service_name="news-service")
        await service_registry.close()
    
    if config_center:
        await config_center.close()
    
    UnifiedLogger.info("News Service shutdown completed")
