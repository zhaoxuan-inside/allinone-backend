...
from src.stocks_service.router import router as stocks_router

app = FastAPI(
    title="Stocks Service",
    description="Stocks Service for AllInOne Platform",
    version="0.1.0"
)
...
async def startup():
    ...
    UnifiedLogger.info("Stocks Service started successfully")
...
async def shutdown():
    ...
    if service_registry:
        await service_registry.deregister_service(service_name="stocks-service")
        await service_registry.close()
    
    if config_center:
        await config_center.close()
    
    UnifiedLogger.info("Stocks Service shutdown completed")


@app.get("/")
async def root():
    return {"message": "Stocks Service"}
