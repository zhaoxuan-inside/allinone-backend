...
from src.forum_service.router import router as forum_router

app = FastAPI(
    title="Forum Service",
    description="Forum Service for AllInOne Platform",
    version="0.1.0"
)
...
async def startup():
    ...
    UnifiedLogger.info("Forum Service started successfully")
...
async def shutdown():
    ...
    if service_registry:
        await service_registry.deregister_service(service_name="forum-service")
        await service_registry.close()
    
    if config_center:
        await config_center.close()
    
    UnifiedLogger.info("Forum Service shutdown completed")


@app.get("/")
async def root():
    return {"message": "Forum Service"}
